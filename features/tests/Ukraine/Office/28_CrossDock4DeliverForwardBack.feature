...
Feature: Check 4 Cross Dock waybills
    ...
    I want to check 4 Cross Dock waybills goes forward and back
        ...
        # создаем накладную
    Scenario Outline: 28_4 Cross Dock Waybills Goes Forward And Back
       ...
       # перевести статус "В перевозке"
        ...
        And I expect this element "Loader" is not visible
        # Выбор направления
        And I choice "<Direction3>" of direction
        #кликнуть на кнопку "Расчитать распределение по ТК" и выбрать тк
        And I click calculate and select "<TK>" transport company
        # перевести статус перевозки "ТС заказано"
        And I moving status forward "ТС заказано"
        ...
        And I expect this element "Loader" is not visible
        # Выбор направления
        And I choice "<Direction4>" of direction
        #кликнуть на кнопку "Расчитать распределение по ТК" и выбрать тк
        And I click calculate and select "<TK>" transport company
        # перевести статус перевозки "ТС заказано"
        And I moving status forward "ТС заказано"
        ...
        Examples:
            | TK      | shipping warehouse                                              | delivery warehouse | delivery warehouse 2 | delivery warehouse 3 | delivery warehouse 4 | vendorcode | number vendorcode | login coordinator         |Direction                     |Direction3                    |Direction4                  |
            | ТОВ ХФК | вул. Садова 26-а, смт. Гостомель, Київська обл., 08290, Україна | S0006              | JJBC01               | JNJKN02              | JJBC01               | 8077501    | 225000            | Ukr Transport Coordinator |Велика Олександрівка~Гостомель|Гостомель~Велика Олександрівка|Велика Олександрівка~Стоянка|
