...
Feature: Check Notifications
    ...
    Scenario Outline: 14_Notification ч1
        ...
        And I click on this element "Редактирование пользователя"
        And I add "testmail1" to the inputfields "inputEmail"
        And I click on this element "buttonSave"
        And I expect this element "Loader" is not visible
        ...
        And I click on this element "Редактирование пользователя"
        And I add "testmail2" to the inputfields "inputEmail"
        And I click on this element "buttonSave"
        And I expect this element "Loader" is not visible
        Then I expects that element "Грид Админки" contains the text "<transport_tk>"
        #добавляем мыло складу отгрузки
        And I add "<comment sklad>" to the inputfields "Поиск по комментарию"
        And I press "Enter"
        ...
        And I click on this element "Редактирование пользователя"
        And I add "testmail3" to the inputfields "inputEmail"
        And I click on this element "buttonSave"
        And I expect this element "Loader" is not visible
        Then I expects that element "Грид Админки" contains the text "<sklad>"
        And I click on this element "Сброс фильтра в админке"
        And I expect this element "Loader" is not visible
        #добавляем мыло складу доставки
        And I add "<comment sklad2>" to the inputfields "Поиск по комментарию"
        And I press "Enter"
        And I add "<sklad2>" to the inputfields "Поиск по логину"
        And I press "Enter"
        And I expect this element "Loader" is not visible
        When I expects that element "Грид Админки" contains the text "<sklad2>"
        And I click on this element "Редактирование пользователя"
        And I add "testmail4" to the inputfields "inputEmail"
        And I click on this element "buttonSave"
        And I expect this element "Loader" is not visible
        Then I expects that element "Грид Админки" contains the text "<sklad2>"
        And I click on this element "Сброс фильтра в админке"
        And I expect this element "Loader" is not visible
        #добавляем мыло клиенту
        And I add "<client>" to the inputfields "Поиск по логину"
        And I press "Enter"
        And I expect this element "Loader" is not visible
        When I expects that element "Грид Админки" contains the text "<client>"
        And I click on this element "Редактирование пользователя"
        And I add "testmail5" to the inputfields "inputEmail"
        And I click on this element "buttonSave"
        And I expect this element "Loader" is not visible
        Then I expects that element "Грид Админки" contains the text "<client>"
        And I open the url "/logout"

        Examples:
            | coordinator    | customer      | transport_tk     | sklad      | sklad2      | client      | comment sklad  | comment sklad2 |
            | test_consumer2 | test_customer | test_transport_k | test_sklad | test_sklad2 | test_client | Склад отгрузки | Склад доставки |

        #заходим юзером с ролью кастомер сервис и создаем накладную
    Scenario Outline:14_Notification ч2
        # create waybills
       ...
       # перевести статус "В перевозке"
        ...
        Examples:
            ...
            | Ителла | CONSMSKFM          | Российская Федерация 440516 Пензенский р-н с. Саловка Полевая ул. д.15 | 1603706S   | 110000            | Consumer Transport Coordinator | Consumer Customer Service | Consumer Sklad | Consumer Sklad2 |

    Scenario: 14_Notification ч3
        # Check mail
        ...
        And I expects that element "Грид Mailnesia" contains the text "Накладная готова к подборке (Artlogics_INFO_#1)"
        ...
        And I add "testmail1" to the inputfields "В поле поиска email"
        When I click on this element "Стрелка входа"
        And I have closed all but the first tab
        And I wait on mailnesia elements "testmail1"
        And I expects that element "Грид Mailnesia" contains the text "Перевозка подтверждена (Artlogics_INFO_#2)"
        And I expects that element "Грид Mailnesia" contains the text "Artlogics_INFO_#5"
        Then I delete all mails in mailnesia

        #проверяем письма у транспортной компании
        And I add "testmail2" to the inputfields "В поле поиска email"
        Then I click on this element "Стрелка входа"
        And I have closed all but the first tab
        And I wait on mailnesia elements "testmail2"
        And I expects that element "Грид Mailnesia" contains the text "Artlogics_INFO_#6"
        Then I delete all mails in mailnesia

       #проверяем письма у склада отгрузки
        And I add "testmail3" to the inputfields "В поле поиска email"
        Then I click on this element "Стрелка входа"
        And I have closed all but the first tab
        And I wait on mailnesia elements "testmail3"
        And I expects that element "Грид Mailnesia" contains the text "Накладная готова к подборке (Artlogics_INFO_#1)"
        And I expects that element "Грид Mailnesia" contains the text "Перевозка подтверждена (Artlogics_INFO_#2)"
        #And I expects that element "Грид Mailnesia" contains the text "Artlogics_INFO_#5"
        Then I delete all mails in mailnesia

        #проверяем письма у склада доставки
        And I add "testmail4" to the inputfields "В поле поиска email"
        When I click on this element "Стрелка входа"
        And I have closed all but the first tab
        And I wait on mailnesia elements "testmail4"
        And I expects that element "Грид Mailnesia" contains the text "Artlogics_INFO_#3"
        Then I delete all mails in mailnesia

        #проверяем письма у клиента
        And I add "testmail5" to the inputfields "В поле поиска email"
        Then I click on this element "Стрелка входа"
        And I have closed all but the first tab
        And I wait on mailnesia elements "testmail5"
        And I expects that element "Грид Mailnesia" contains the text "Artlogics_INFO_#22"
        Then I delete all mails in mailnesia
