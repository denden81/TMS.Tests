...
Feature: Check  status Idle in one waybills
    As a admin
    I want to check status idle waybills

        # создаем накладную
    Scenario Outline: 23_Check Status Idle One Waybills:Холостая подача
       ...
       # перевести статус "В перевозке"
        ...
        # перевести статус перевозки "Холостая подача"
        When I click on this element "Холостая подача в перевозке"
        ...
        # кликнуть на линк перевозки
        And I scroll to elements "Линк перевозки в статусе Холостая подача"
        And I click on this element "Линк перевозки в статусе Холостая подача"
        And I expect this element "Loader" is not visible
