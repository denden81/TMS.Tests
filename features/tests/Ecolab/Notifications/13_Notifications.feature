...
Feature: Check Notifications
    ...
    Scenario Outline: 13_Check Notifications: ч1
        ...
        And I click on this element "Редактирование пользователя"
        And I add "testmail8" to the inputfields "inputEmail"
        And I click on this element "buttonSave"
        And I expect this element "Loader" is not visible
        ...
        And I click on this element "Редактирование пользователя"
        And I add "testmail9" to the inputfields "inputEmail"
        And I click on this element "buttonSave"
        And I expect this element "Loader" is not visible
        ...
        And I click on this element "Редактирование пользователя"
        And I add "testmail10" to the inputfields "inputEmail"
        And I click on this element "buttonSave"
        And I expect this element "Loader" is not visible
       ...
       And I open the url "/logout"
        ...
        Examples:
            ...
            | test_ecolab2 | test_ecolab_customer | test_ecolab_transport_tk | test_ecolab_sklad | test_ecolab_sklad2 | Склад отгрузки | Склад доставки |

        #заходим юзером с ролью кастомер сервис и создаем накладную
    Scenario Outline:13_Check Notifications: ч2
        #create waybills
       ...
       # перевести статус "В перевозке"
        ...
        Examples:
            ...
            | Автобал | 011_M0000000510    | Биосинтез          | 0193537    | 120               | Ecolab Transport Coordinator | Ecolab Customer Service | Ecolab Sklad | Ecolab Sklad2 |

    Scenario:13_Check Notifications: ч3
        # Check mail
        And I open the sites "Майлнезия"
        #проверяем письма у транспортного координатора
        And I add "testmail6" to the inputfields "В поле поиска email"
        When I click on this element "Стрелка входа"
        And I have closed all but the first tab
        And I wait on mailnesia elements "testmail6"
        ...
        And I add "testmail7" to the inputfields "В поле поиска email"
        When I click on this element "Стрелка входа"
        And I have closed all but the first tab
        And I wait on mailnesia elements "testmail7"
        And I expects that element "Грид Mailnesia" contains the text "Перевозка подтверждена (Artlogics_INFO_#2)"
        And I expects that element "Грид Mailnesia" contains the text "Artlogics_INFO_#5"
        Then I delete all mails in mailnesia

        #проверяем письма у транспортной компании
        And I add "testmail8" to the inputfields "В поле поиска email"
        Then I click on this element "Стрелка входа"
        And I have closed all but the first tab
        And I wait on mailnesia elements "testmail8"
        And I expects that element "Грид Mailnesia" contains the text "Artlogics_INFO_#6"
        Then I delete all mails in mailnesia

       #проверяем письма у склада отгрузки
        And I add "testmail9" to the inputfields "В поле поиска email"
        Then I click on this element "Стрелка входа"
        And I have closed all but the first tab
        And I wait on mailnesia elements "testmail9"
        And I expects that element "Грид Mailnesia" contains the text "Накладная готова к подборке (Artlogics_INFO_#1)"
        And I expects that element "Грид Mailnesia" contains the text "Перевозка подтверждена (Artlogics_INFO_#2)"
        And I expects that element "Грид Mailnesia" contains the text "Artlogics_INFO_#5"
        Then I delete all mails in mailnesia
        ...
        And I add "testmail10" to the inputfields "В поле поиска email"
        When I click on this element "Стрелка входа"
        And I have closed all but the first tab
        And I wait on mailnesia elements "testmail10"
