...
Feature: Check waybills
    As a admin
    I want to check View Broker

        #создаем накладную
    Scenario Outline: 04_View Broker
       ...
       # And I expect this element "Loader" is not visible
        ...
        Then I click on this element "Иконка выбора брокера"
        And I click on this element "Выбрать 1го брокера"
        And I expect this element "Loader" is not visible
        And I open the url "/logout"

         #заходим юзером с ролью брокер1 и смотрим что накладная есть
        Given I log in as a "<login broker>" on homepage
        And I expect this element "Loader" is not visible
        And I wait on elements "Office"
        When I click on this element "RemoveFilter"
        And I wait on elements "Office"
        And I check filter "random number" deliveryNumber
        And I expects that element "scrollContainer" contains the text "random number"
        And I open the url "/logout"

        #заходим юзером с ролью брокер2 и смотрим что накладной нет
        Given I log in as a "<login broker2>" on homepage
        And I expect this element "Loader" is not visible
        And I wait on elements "Office"
        When I click on this element "RemoveFilter"
        And I wait on elements "Office"
        And I check filter "random number" deliveryNumber
        And I wait the element "Накладная ИЕ" to not be checked

        Examples:
            | TK     | shipping warehouse | delivery warehouse | vendorcode            | number vendorcode | login coordinator        | login broker | login broker2 |
            | Ителла | Ethicon LLC Brazil        | MD Лесной          | TRILOC II CUP 32/44mm | 47500             | MD Transport Coordinator | MD broker    | MD broker2    |
