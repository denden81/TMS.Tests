...
Feature: Check Notifications
    ...
    Scenario Outline: 12_Notifications: ч1
        ...
        And I add "testmail20" to the inputfields "inputEmail"
        ...
        And I click on this element "Редактирование пользователя"
        And I add "testmail21" to the inputfields "inputEmail"
        And I click on this element "buttonSave"
        And I expect this element "Loader" is not visible
        ...
        And I click on this element "Редактирование пользователя"
        And I add "testmail22" to the inputfields "inputEmail"
        And I click on this element "buttonSave"
        And I expect this element "Loader" is not visible
        ...
        And I click on this element "Редактирование пользователя"
        And I add "testmail23" to the inputfields "inputEmail"
        And I click on this element "buttonSave"
        And I expect this element "Loader" is not visible
       ...
       And I open the url "/logout"
        ...
        Examples:
            ...
            | test_vistakon2 | test_vistakon_customer | test_vistakon_transport_tk | test_vis_sklad | Склад отгрузки |

        #заходим юзером с ролью кастомер сервис и создаем накладную
    Scenario Outline: 12_Notifications: ч2
        # create waybills
       ...
       # перевести статус "В перевозке"
        ...
        Examples:
            ...
            | Даймекс | VC3000             | VC3001             | 733905098321 | 67000             | Vis Transport Coordinator | Vis Customer Service | Vis Sklad   | Vis Sklad2   |

    Scenario: 12_Notifications: ч3
        # Check mail
        And I open the sites "Майлнезия"
        #проверяем письма у транспортного координатора
        And I add "testmail20" to the inputfields "В поле поиска email"
        When I click on this element "Стрелка входа"
        And I have closed all but the first tab
        And I wait on mailnesia elements "testmail20"
        ...
        And I add "testmail21" to the inputfields "В поле поиска email"
        When I click on this element "Стрелка входа"
        And I have closed all but the first tab
        And I wait on mailnesia elements "testmail21"
        And I expects that element "Грид Mailnesia" contains the text "Перевозка подтверждена (Artlogics_INFO_#2)"
        And I expects that element "Грид Mailnesia" contains the text "Artlogics_INFO_#5"
        Then I delete all mails in mailnesia

        #проверяем письма у транспортной компании
        And I add "testmail22" to the inputfields "В поле поиска email"
        Then I click on this element "Стрелка входа"
        And I have closed all but the first tab
        And I wait on mailnesia elements "testmail22"
        And I expects that element "Грид Mailnesia" contains the text "Artlogics_INFO_#6"
        Then I delete all mails in mailnesia

       #проверяем письма у склада отгрузки
        And I add "testmail23" to the inputfields "В поле поиска email"
        Then I click on this element "Стрелка входа"
        And I have closed all but the first tab
        And I wait on mailnesia elements "testmail23"
        And I expects that element "Грид Mailnesia" contains the text "Artlogics_INFO_#5"
        Then I delete all mails in mailnesia
