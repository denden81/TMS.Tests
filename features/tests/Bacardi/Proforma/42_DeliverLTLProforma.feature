...
Feature: Check Deliver LTL Proforma
    ...
    I want to check waybills Deliver LTL Proforma
        ...
        # создаем накладную
    Scenario Outline: 42_Deliver LTL Proforma
       ...
       # перевести статус "В перевозке"
        ...
        And I check rate "40 000",rate simple "0" in LTL Proforma

        Examples:
            | TK       | shipping warehouse | delivery warehouse | vendorcode | number vendorcode | login coordinator         | type tc                      |
            | ЛТ Групп | Склад Истры        | Пенза              | 1102001281 | 7000              | Bac Transport Coordinator | паллетная доставка (изотерм) |
