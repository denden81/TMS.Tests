...
Feature: Check search driver
    As a Transport Coordinator
    I want to check search driver

    Scenario Outline: 09_Search Driver
        ...
        #проверяем поиск по Экспедитору
        Then I click on this element "RemoveFilter"
        And I expect this element "Loader" is not visible
        And I add "<freithforwader>" to the inputfields "Поле поиска по Экспедитору"
        And I expect this element "Loader" is not visible
        And I expects that element "footerGridDriver" contains the text "<freithforwader>"

        #проверяем поиск по Комментарию
        When I click on this element "RemoveFilter"
        And I expect this element "Loader" is not visible
        And I add "<comment>" to the inputfields "Поле поиска по Комментарий Бакарди"
        And I expect this element "Loader" is not visible
        ...
        And I click on this element "Поиск Транспортной компании у водителя Бакарди"
        And I add "<TK>" to the inputfields "Поиск Транспортной компании водителя Бакарди"
        ...
        Examples:
            | driver telefon | TK       | driver passport | issue by   | number license | data license | attorney no | comment               | freithforwader   |
            | +79020304051      | ЛТ Групп | 5845701020      | ОВД Москва | 45127845       | 10/10/2020   | 45127846    | самый лучший водитель | Водитель котейка |
