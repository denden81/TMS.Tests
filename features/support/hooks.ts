   ...
   // before: function (capabilities, specs) {
    ...
    afterStep: function (
        ...
        if (passed === false) {
            ...
            const rnd1 = getRandomInt(1, 99999);
            let date = new Date();
            let datadT, datayy, monthD, datadD, datadd;
            datadT = date.getTime();
            datayy = date.getFullYear();
            monthD = (date.getMonth() + 1);
            datadD = date.getDate();
            datadd = (datadT + "_" + datadD + "." + monthD + "." + datayy + "_");
            let text = "reports/screenshots/" + datadd + "" + rnd1 + ".png"
            browser.saveScreenshot(text);
        }
    },
    /**
